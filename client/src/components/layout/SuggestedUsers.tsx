...
export function SuggestedUsers() {
  ...
  useEffect(() => {
    const fetchSuggestedUsers = async () => {
      ...
      try {
        setLoading(true);
        const response = await fetch(`http://localhost:3002/api/user/suggested/${user.id}`, {
          ...
          users.map((suggestedUser) => (
            ...
            >
              <div className="flex items-center gap-1">
                ...
                />
                <div>
                  <p className="font-medium text-xs text-blue-950">{suggestedUser.full_name}</p>
                  <p className="text-xs text-white">@{suggestedUser.username}</p>
                </div>
              </div>
              <button
